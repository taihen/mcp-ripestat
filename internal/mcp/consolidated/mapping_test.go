...
func TestMapStringsToOperations(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			mappingKey: "analysis",
			want:       []Operation{OpConsistency, OpPathOptimization, OpUpdates},
			wantErr:    false,
		},
		{
			name:       "data - whois",
		...
		},
		{
			...
			mappingKey: "data",
			want:       []Operation{OpWhois, OpAllocationHistory, OpHierarchy, OpContacts},
			wantErr:    false,
		},
		{
			...
			mappingKey: "checks",
			want:       []Operation{OpRPKI},
			wantErr:    false,
		},
		{
			name:       "checks preserve distinct semantics",
			values:     []string{SecurityCheckRPKI, SecurityCheckAbuseContacts, SecurityCheckBGPHijacking},
			mappingKey: "checks",
			want:       []Operation{OpRPKI, OpAbuseContacts, OpBGPHijacking},
			wantErr:    false,
		},
		{
			...
			want:       []Operation{OpNeighbors, OpAnnouncedPrefixes, OpRelatedNetworks},
			wantErr:    false,
		},
		{
