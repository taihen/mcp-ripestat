...
const (
	// EndpointPath is the path to the RIPEstat data API for routing status.
	EndpointPath = "/data/routing-status/data.json"
)
...
func (c *Client) Get(ctx context.Context, resource string) (*Response, error) {
	...
	if err := c.client.GetJSON(ctx, EndpointPath, params, &response); err != nil {
		return nil, errors.ErrServerError.WithError(fmt.Errorf("failed to get routing status: %w", err))
	}

	return &response, nil
}

// GetRoutingStatus is a convenience function that uses the default client to get routing status information.
func GetRoutingStatus(ctx context.Context, resource string) (*Response, error) {
