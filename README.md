...
`mcp-ripestat` is a Go-based, locally run MCP server that fetches and serves
data from the RIPEstat Data API.
...
This software is currently under active development. It may not be stable and is
...
To integrate the server with Cursor (and other MCP clients), add the following
configuration to your global `mcp.json` file (e.g., `~/.cursor/mcp.json` on
macOS/Linux).
...
After adding this and restarting Cursor, you can test it with a prompt like:
